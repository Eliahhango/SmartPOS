  ...
  return (
    <div className="space-y-4">
        ...
        <>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <table className="w-full text-sm">
              <thead><tr className="text-left text-gray-500 bg-gray-50"><th className="p-4">Product</th><th className="p-4">Category</th><th className="p-4">Stock</th><th className="p-4">Min Stock</th><th className="p-4">Status</th><th className="p-4">Expiry</th></tr></thead>
              <tbody>
                {products.map(p => (
                  <tr key={p.id} className="border-t border-gray-50">
                    ...
                    <td className="p-4 text-gray-500">{p.minimumStock}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-xs text-gray-400">{p.expiryDate ? new Date(p.expiryDate).toLocaleDateString() : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === 'movements' && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-gray-500 bg-gray-50"><th className="p-4">Date</th><th className="p-4">Product</th><th className="p-4">Change</th><th className="p-4">Reason</th><th className="p-4">User</th><th className="p-4">Notes</th></tr></thead>
            <tbody>
              {movements.map(m => (
                <tr key={m.id} className="border-t border-gray-50">
                  ...
                  <td className="p-4">
                    <span className={`flex items-center gap-1 font-semibold ${m.changeQty > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {m.changeQty > 0 ? <ArrowUp size={14} /> : <ArrowDown size={14} />}
                      {Math.abs(m.changeQty)}
                    </span>
                  </td>
                  <td className="p-4"><span className="px-2 py-0.5 bg-gray-100 rounded-full text-xs capitalize">{m.reason}</span></td>
                  <td className="p-4 text-gray-500">{m.user?.name || '-'}</td>
                  <td className="p-4 text-xs text-gray-400">{m.notes || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
