...
export default function PurchasesPage() {
  ...
  const statusColors: any = { draft: 'bg-gray-100 text-gray-700', approved: 'bg-indigo-100 text-indigo-700', received: 'bg-green-100 text-green-700' };
        ...
        {purchases.map(p => (
          <div key={p.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <span className="font-bold text-indigo-600">{formatCurrency(p.totalAmount)}</span>
                <button onClick={() => setSelected(p)} className="p-1.5 hover:bg-gray-100 rounded-lg"><Eye size={16} /></button>
                {p.status === 'draft' && <button onClick={() => handleApprove(p.id)} className="px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-medium flex items-center gap-1"><Check size={14} /> Approve</button>}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 animate-zoom-in max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between"><span className="text-sm font-medium">Items</span><button type="button" onClick={addItem} className="text-xs text-indigo-600">+ Add Item</button></div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="flex-1 py-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl text-sm font-semibold">Create PO</button>
              </div>
            </form>
          </div>
        </div>
