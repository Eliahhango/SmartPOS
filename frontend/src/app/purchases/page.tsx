...
export default function PurchasesPage() {
  ...
  const statusColors: any = { draft: 'bg-slate-100 text-slate-700', approved: 'bg-indigo-100 text-indigo-700', received: 'bg-emerald-100 text-emerald-700' };
    ...
    <div className="space-y-4">
          ...
          <div key={p.id} className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Invoice: {p.invoiceNo || 'N/A'} • {formatDate(p.date)}</p>
              </div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-teal-600">{formatCurrency(p.totalAmount)}</span>
                <button onClick={() => setSelected(p)} className="p-1.5 hover:bg-slate-100 rounded-lg"><Eye size={16} /></button>
                {p.status === 'draft' && <button onClick={() => handleApprove(p.id)} className="px-3 py-1.5 bg-teal-50 text-indigo-700 rounded-lg text-xs font-medium flex items-center gap-1"><Check size={14} /> Approve</button>}
                {p.status === 'approved' && <button onClick={() => handleReceive(p.id)} className="px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-medium flex items-center gap-1"><Truck size={14} /> Receive</button>}
              </div>
            </div>
            <div className="flex gap-2 flex-wrap">
              {p.items?.map((item: any) => (
                <span key={item.id} className="text-xs bg-slate-50 px-2 py-1 rounded-lg">{item.product?.name} × {item.quantity}</span>
              ))}
            </div>
          </div>
        ))}
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-lg p-6 animate-zoom-in">
            ...
            <table className="w-full text-sm">
              <thead><tr className="text-left text-slate-500"><th className="pb-2">Product</th><th className="pb-2 text-right">Qty</th><th className="pb-2 text-right">Cost</th><th className="pb-2 text-right">Total</th></tr></thead>
              <tbody>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-lg p-6 animate-zoom-in max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between"><span className="text-sm font-medium">Items</span><button type="button" onClick={addItem} className="text-xs text-teal-600">+ Add Item</button></div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="flex-1 py-2 bg-gradient-to-r from-teal-500 to-emerald-500 text-white rounded-xl text-sm font-semibold">Create PO</button>
              </div>
            </form>
          </div>
        </div>
